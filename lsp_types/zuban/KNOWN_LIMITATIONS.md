...
**Limitation**: Zuban does not yet report unused `# type: ignore` comments (upstream limitation still present as of Zuban 0.9.1, per the [features documentation](https://docs.zubanls.com/en/latest/features.html)).
...
**Limitation**: Zuban does not currently type-check function bodies that use bounded `TypeVar` definitions such as `TypeVar("T", str, bytes)` (upstream limitation still present as of Zuban 0.9.1).
...
These limitations were documented based on Zuban version 0.7.0 (April 2026), last verified with Zuban 0.9.1 (August 2026). Both behavioral caveats were re-probed directly: an unused `# type: ignore` produces no diagnostic, and a type error inside a bounded-`TypeVar` function body goes unreported while the same error in a plain function is caught. Future versions may address some of these limitations.
